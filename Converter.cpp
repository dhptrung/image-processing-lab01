...
int Converter::RGB2GrayScale(const Mat& sourceImage, Mat& destinationImage) 
{
	...
	// Dùng con trỏ đầu dòng để truy xuất đến từng pixel
	for (int y = 0; y < height; y++) 
	{
		...
		for (int x = 0; x < width; x++, pSrcRow += srcChannels, pDstRow += dstChannels) 
		{
			...
			// Gán giá trị gray cho pixel của destinationImage
			pDstRow[0] = gray_val;
		}
	}
...
int Converter::GrayScale2RGB(const Mat& sourceImage, Mat& destinationImage) 
{
	...
	for (int y = 0; y < height; y++) 
	{
		...
		for (int x = 0; x < width; x++, pSrcRow += srcChannels, pDstRow += dstChannels) 
		{
			// Lấy giá trị gray từ sourceImage (ảnh GrayScale)
			uchar gray_val = pSrcRow[0];
			// Gán giá trị gray cho từng channel B, R, G của pixel destinationImage
			pDstRow[0] = gray_val; // B
			pDstRow[1] = gray_val; // G
			pDstRow[2] = gray_val; // R
		}
	}
